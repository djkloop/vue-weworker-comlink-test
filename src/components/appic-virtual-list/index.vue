 ...
 * @LastEditors   : djkloop
 * @LastEditTime  : 2021-04-13 11:03:15
 * @Description   : 头部注释
 * @FilePath      : /appiccms/src/views/components/appic-virtual-list/index.vue
-->
<template>
  <div ref="parent" class="appic-infinite-list-parent">
    <div
      ref="list"
      :style="{ height: height + 'px' }"
      class="appic-infinite-list-container"
      @scroll="scrollEvent($event)"
    >
      <div ref="phantom" class="appic-infinite-list-phantom"></div>
      <!-- 顶部刷新区域 -->
      <div
        ref="top"
        v-if="topLoadMore"
        v-show="dargState !== 'none' && touchDistance >= 20"
        class="appic-infinite-top-container"
      >
        <slot name="top" :dargState="dargState" :dargDistance="touchDistance">
          <div
            class="appic-infinite-top-content"
            :style="{ color: topTextColor }"
          >
            <span
              class="appic-infinite-top-content-icon icon-arrow"
            ...
            <span
              class="appic-infinite-top-content-icon icon-revolve"
              v-if="dargState === 'loading'"
            >
              <svg
                class="icon"
                t="1572936012117"
            ...
            <span class="appic-infinite-top-content-title">
      ...
      <div ref="content" class="appic-infinite-list">
        <div
          ref="items"
          class="appic-infinite-list-item-container"
          :id="row._key"
          :key="row._key"
          v-for="(row, row_index) in visibleData"
        >
          ...
          <template v-if="row.value.length < column">
            <div
              v-for="(item, index) in column - (row.value.length % column)"
              class="appic-infinite-item"
              :key="'empty-' + index"
            ></div>
          </template>
        </div>
...
export default {
  ...
  computed: {
    ...
    visibleCount() {
      if (this.screenHeight === 0) {
        return Math.ceil(this.height / this.estimatedItemSize);
      }
  ...
  mounted() {
    this.screenHeight = this.$el.clientHeight || this.height;
    this.start = 0;
  ...
  methods: {
    ...
    touchMoveEvent(event) {
      ...
      if (curPos > this._prevPos && this.$refs.list.scrollTop === 0) {
        // event.preventDefault();
        this.touchDistance = Math.max(
          this.touchDistance + curPos - this._prevPos,
          0
        );
    ...
    touchEndEvent() {
      ...
      this.$emit(
        "top-status-change",
        this.dargState,
        ~~(this.touchDistance / this.distanceScale)
      );
...
.appic-infinite-top-content-icon {
  display: flex;
  align-items: center;
...
.appic-infinite-top-content-title {
  font-size: 14px;
}

.appic-infinite-top-container {
  transform: translateZ(0);
  position: relative;
  z-index: 2;
}

.appic-infinite-list-container {
  overflow-x: hidden;
  width: 100%;
  overflow-y: auto;
  position: relative;
